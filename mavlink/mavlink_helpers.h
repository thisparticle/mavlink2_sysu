					   ...
					   const uint8_t crc[2])
{
	...
	//printf("\n T_header:\n");
	//for(int i=0;i<header_len;i++) printf(" -%x- ",header[i]);
	//加入payload帧
	mavlink_sha256_update(&ctx, packet, packet_len);
	//printf("+%d+",packet_len);
	//printf("\n T_packet:\n");
	//for(int i=0;i<packet_len;i++) printf(" -%x- ",packet[i]);
	//加入CRC？
	mavlink_sha256_update(&ctx, crc, 2);
	//printf("\n T_crc:\n");
	//for(int i=0;i<2;i++) printf(" -%x- ",crc[i]);	
	//printf("---%x---",packet[0]);
	//加入link_id及时间戳
	mavlink_sha256_update(&ctx, signature, 7);
	//printf("\n T_link_id&timestamp:\n");
	//for(int i=0;i<7;i++) printf(" -%x- ",signature[i]);	
	
	//生成最终的48位密码，6个byte，并存入了签名帧中
	mavlink_sha256_final_48(&ctx, &signature[7]);
	//printf("\n T_signature:\n");
	//for(int i=0;i<MAVLINK_SIGNATURE_BLOCK_LEN;i++) printf(" -%x- ",signature[i]);	
					    ...
					    const mavlink_message_t *msg)
{
	...
	//printf("%d:%d:%d:%d:%d:%d:",psig[0],psig[1],psig[2],psig[3],psig[4],psig[5]);
	//printf("\n psig:\n");
	//for(int i = 0; i < 13; i++) printf("%02X ",psig[i]);
	//printf("\n");
	//msg的签名帧校验位
	...
	//printf("\n R_header&paylod:\n");
	//for(int i=0;i<MAVLINK_CORE_HEADER_LEN+1+msg->len;i++) printf(" -%x- ",p[i]);
	//加入CRC？
//	uint8_t crc[2];
//	crc[0] = 0x4d;
//	crc[1] = 0x86;
	//mavlink_sha256_update(&ctx, crc, 2);//checksum
	mavlink_sha256_update(&ctx, msg->ck, 2);//checksum
	//mavlink_sha256_update(&ctx, msg->checksum, 2);//checksum
	//printf("\n R_crc:\n");
	//for(int i=0;i<2;i++) printf(" -%x- ",msg->ck[i]);
	//加入link_id及时间戳
	mavlink_sha256_update(&ctx, psig, 1+6);
	//printf("\n R_linkid&timestamp:\n");
	//for(int i=0;i<7;i++) printf(" -%x- ",psig[i]);
	//将计算出来的签名存入signature
	mavlink_sha256_final_48(&ctx, signature);
	//printf("\n R_signature:\n");
	//for(int i=0;i<6;i++) printf(" -%x- ",signature[i]);
	
 /**************************************************************to       do*****************************/
	if (memcmp(signature, incoming_signature, 6) != 0) {
		printf("Signature ： no ! \n");
		return false;
		//签名不过直接就return false了
	}else
	{
						      ...
						      mavlink_status_t* status, uint8_t min_length, uint8_t length, uint8_t crc_extra)
{
	...
	//签名验证不过是因为msg->ck均为0，在校验计算是签名计算错误，设置msg->ck解决签名不过的bug，/*czq,2020/12/1*/
	//mavlink_ck_0(msg) = (uint8_t)(checksum & 0xFF);
	//mavlink_ck_1(msg) = (uint8_t)(checksum >> 8);
	msg->ck[0] = (uint8_t)(checksum & 0xFF);
	msg->ck[1] = (uint8_t)(checksum >> 8);
						      ...
						      uint8_t chan, uint8_t min_length, uint8_t length, uint8_t crc_extra)
{
	...
	//printf("key1 ");
	//for(int i=0;i<32;i++) printf("%02X ",secret_sign_key_1[i]);

	//link id赋值
	signing.link_id = (uint8_t)chan;
	//时间戳赋值，单位是ms
	//uint64_t timestamp_now = get_time_msec();
	//uint64_t timestamp_now = 1;
	uint64_t timestamp_now = getRTCAsUnixTime_m();
	signing.timestamp = timestamp_now; 
	//signing.timestamp = 0.0; 
	//printf("\n decode_time:%llu",timestamp_now);
